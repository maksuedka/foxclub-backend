...
- Логика:
  ...
  - изображения загружаются во внешнее хранилище через `ImageKitService` (используется на backend)
...
**Комментарии к постам (в рамках фронта):**
- При рендере карточек и ленты фронт дополнительно делает `GET /api/comments/post/{postId}` для подгрузки списка комментариев и расчета `commentCount` (счетчика)
...
- `src/main/resources/static/js/posts.js`
  ...
  - для выбранных постов подгружаются комментарии и показывается счетчик (`commentCount`).
  - базовый URL определяется автоматически (локально/на хостинге) через `getBaseUrl()`.
